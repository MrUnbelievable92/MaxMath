...
{
    unsafe public static class t_long4
    {
        ...
        {
            long4 x = new long4(TestData_LHS[0].x);
        ...
        public static void op_Addition()
        {
            ...
            {
                long4 x = TestData_LHS[i] + TestData_RHS[i];

                result &= x.x == (long)(TestData_LHS[i].x + TestData_RHS[i].x) &
                          x.y == (long)(TestData_LHS[i].y + TestData_RHS[i].y) &
                          x.z == (long)(TestData_LHS[i].z + TestData_RHS[i].z) &
                          x.w == (long)(TestData_LHS[i].w + TestData_RHS[i].w);
            }
        ...
        public static void op_Division()
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    Assert.AreEqual(l[j] / r[j], test[j]);
                }
            }
        }
        ...
        public static void Cast_ToV256()
        {
            ...
            if (Avx2.IsAvx2Supported)
            {
                for (int i = 0; i < NUM_TESTS; i++)
                {
                    Unity.Burst.Intrinsics.v256 x = TestData_LHS[i];

                    result &= x.SLong0 == TestData_LHS[i].x &
                              x.SLong1 == TestData_LHS[i].y &
                              x.SLong2 == TestData_LHS[i].z &
                              x.SLong3 == TestData_LHS[i].w;
                }
            }
        ...
        public static void Cast_FromV256()
        {
            ...
            if (Avx2.IsAvx2Supported)
            {
                ...
                {
                    long4 x = TestData_LHS[i];
                    Unity.Burst.Intrinsics.v256 c = x;
                    x = c;
        ...
        public static void Cast_ToInt()
        {
            ...
            for (int i = 0; i < NUM_TESTS; i++)
            {
                int4 x = (int4)TestData_LHS[i];

                result &= x.x == (int)TestData_LHS[i].x &
                          x.y == (int)TestData_LHS[i].y &
                          x.z == (int)TestData_LHS[i].z &
                          x.w == (int)TestData_LHS[i].w;
            }
        ...
        [Test]
        public static void Cast_FromFloat()
        {
            Random32 rng = Random32.New;

            for (int i = 0; i < 64; i++)
            {
                float4 x = rng.NextFloat4(long.MinValue + 1e10f, long.MaxValue - 1e10f);
                long4 test = (long4)x;

                for (int j = 0; j < 4; j++)
                {
                    Assert.AreEqual(test[j], (long)x[j]);
                }
            }
        }

        [Test]
        public static void Cast_FromDouble()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 64; i++)
            {
                double4 x = rng.NextDouble4(long.MinValue + 1e10d, long.MaxValue - 1e10d);
                long4 test = (long4)x;
