using NUnit.Framework;


namespace MaxMath.Tests
{
    unsafe public static class t_ulong4x4
    {
        private const int COLUMNS = 4;
        private const int ROWS = 4;


        [Test]
        public static void op_Increment()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 test = l;
                test++;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] + 1);
                }
            }
        }

        [Test]
        public static void op_Decrement()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 test = l;
                test--;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] - 1);
                }
            }
        }

        [Test]
        public static void op_Addition()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l + r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] + r[j]);
                }
            }
        }

        [Test]
        public static void op_Subtraction()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l - r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] - r[j]);
                }
            }
        }

        [Test]
        public static void op_Multiply()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l * r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] * r[j]);
                }
            }
        }

        [Test]
        public static void op_Multiply_ulong()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong r = rng.NextULong();

                ulong4x4 testL = l * r;
                ulong4x4 testR = r * l;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(testL[j], l[j] * r);
                    Assert.AreEqual(testR[j], r * l[j]);
                }
            }
        }

        [Test]
        public static void op_Division()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                for (int j = 0; j < COLUMNS; j++)
                {
                    r[j] = math.select(r[j], 1, r[j] == 0);
                }

                ulong4x4 test = l / r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] / r[j]);
                }
            }
        }

        [Test]
        public static void op_Division_ulong()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong r = rng.NextULong();
                r = (ulong)(r == 0 ? 1 : r);

                ulong4x4 testL = l / r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(testL[j], l[j] / r);
                }
            }

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                for (int j = 0; j < COLUMNS; j++)
                {
                    r[j] = math.select(r[j], 1, r[j] == 0);
                }

                ulong l = rng.NextULong();

                ulong4x4 testR = l / r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(testR[j], l / r[j]);
                }
            }
        }

        [Test]
        public static void op_Modulus()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                for (int j = 0; j < COLUMNS; j++)
                {
                    r[j] = math.select(r[j], 1, r[j] == 0);
                }

                ulong4x4 test = l % r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] % r[j]);
                }
            }
        }

        [Test]
        public static void op_Modulus_ulong()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong r = rng.NextULong();
                r = (ulong)(r == 0 ? 1 : r);

                ulong4x4 testL = l % r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(testL[j], l[j] % r);
                }
            }

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                for (int j = 0; j < COLUMNS; j++)
                {
                    r[j] = math.select(r[j], 1, r[j] == 0);
                }

                ulong l = rng.NextULong();

                ulong4x4 testR = l % r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(testR[j], l % r[j]);
                }
            }
        }

        [Test]
        public static void op_BitwiseAnd()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l & r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] & r[j]);
                }
            }
        }

        [Test]
        public static void op_BitwiseOr()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l | r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] | r[j]);
                }
            }
        }

        [Test]
        public static void op_OnesComplement()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = ~l;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], ~l[j]);
                }
            }
        }

        [Test]
        public static void op_ExclusiveOr()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                ulong4x4 test = l ^ r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] ^ r[j]);
                }
            }
        }

        [Test]
        public static void op_LeftShift_int()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                int r = (int)rng.NextULong(0, 64);

                ulong4x4 test = l << r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] << r);
                }
            }
        }

        [Test]
        public static void op_RightShift_int()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                int r = (int)rng.NextULong(0, 64);

                ulong4x4 test = l >> r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], l[j] >> r);
                }
            }
        }


        [Test]
        public static void op_Equality()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l == r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] == r[j]));
                }
            }
        }

        [Test]
        public static void op_Inequality()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l != r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] != r[j]));
                }
            }
        }

        [Test]
        public static void op_LessThan()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l < r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] < r[j]));
                }
            }
        }

        [Test]
        public static void op_GreaterThan()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l > r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] > r[j]));
                }
            }
        }

        [Test]
        public static void op_LessThanOrEqual()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l <= r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] <= r[j]));
                }
            }
        }

        [Test]
        public static void op_GreaterThanOrEqual()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool4x4 test = l >= r;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (bool4)(l[j] >= r[j]));
                }
            }
        }


        [Test]
        public static void op_ConvertFrom_ulong()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong x = rng.NextULong();

                ulong4x4 test = x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    for (int k = 0; k < ROWS; k++)
                    {
                        Assert.AreEqual(test[j][k], x);
                    }
                }
            }
        }

        [Test]
        public static void op_ConvertFrom_long4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                long4x4 x = new long4x4(rng.NextLong4(), rng.NextLong4(), rng.NextLong4(), rng.NextLong4());

                ulong4x4 test = (ulong4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (ulong4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertFrom_int4x4()
        {
            Random32 rng = Random32.New;

            for (int i = 0; i < 16; i++)
            {
                int4x4 x = new int4x4(rng.NextInt4(), rng.NextInt4(), rng.NextInt4(), rng.NextInt4());

                ulong4x4 test = (ulong4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (ulong4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertFrom_uint4x4()
        {
            Random32 rng = Random32.New;

            for (int i = 0; i < 16; i++)
            {
                uint4x4 x = new uint4x4(rng.NextUInt4(), rng.NextUInt4(), rng.NextUInt4(), rng.NextUInt4());

                ulong4x4 test = (ulong4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (ulong4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertFrom_float4x4()
        {
            Random32 rng = Random32.New;

            for (int i = 0; i < 16; i++)
            {
                float4x4 x = new float4x4(rng.NextFloat4(0, 2f * ulong.MaxValue), rng.NextFloat4(0, 2f * ulong.MaxValue), rng.NextFloat4(0, 2f * ulong.MaxValue), rng.NextFloat4(0, 2f * ulong.MaxValue));

                ulong4x4 test = (ulong4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (ulong4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertFrom_double4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                double4x4 x = new double4x4(rng.NextDouble4(0, Helper.MAX_MONO_RUNTIME_CVT_DOUBLE_LONG + 1), rng.NextDouble4(0, Helper.MAX_MONO_RUNTIME_CVT_DOUBLE_LONG + 1), rng.NextDouble4(0, Helper.MAX_MONO_RUNTIME_CVT_DOUBLE_LONG + 1), rng.NextDouble4(0, Helper.MAX_MONO_RUNTIME_CVT_DOUBLE_LONG + 1));

                ulong4x4 test = (ulong4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (ulong4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertTo_int4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 x = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                int4x4 test = (int4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (int4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertTo_uint4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 x = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                uint4x4 test = (uint4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (uint4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertTo_float4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 x = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                float4x4 test = (float4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (float4)x[j]);
                }
            }
        }

        [Test]
        public static void op_ConvertTo_double4x4()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 x = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                double4x4 test = (double4x4)x;

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], (double4)x[j]);
                }
            }
        }


        [Test]
        public static void Equals()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4x4 l = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());
                ulong4x4 r = new ulong4x4(rng.NextULong4(), rng.NextULong4(), rng.NextULong4(), rng.NextULong4());

                bool test = l.Equals(r);
                bool cmp = true;

                for (int j = 0; j < COLUMNS; j++)
                {
                    cmp &= l[j].Equals(r[j]);
                }

                Assert.AreEqual(test, cmp);
                Assert.IsTrue(l.Equals(l));
            }
        }


        [Test]
        public static void get_Item_int()
        {
            Random64 rng = Random64.New;

            for (int i = 0; i < 16; i++)
            {
                ulong4* columns = stackalloc ulong4[COLUMNS];

                for (int j = 0; j < COLUMNS; j++)
                {
                    columns[i] = rng.NextULong4();
                }

                ulong4x4 test = new ulong4x4(columns[0], columns[1], columns[2], columns[3]);

                for (int j = 0; j < COLUMNS; j++)
                {
                    Assert.AreEqual(test[j], columns[j]);
                }
            }
        }
    }
}