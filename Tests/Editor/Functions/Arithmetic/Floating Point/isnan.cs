using NUnit.Framework;
using Unity.Mathematics;

using static MaxMath.maxmath;
using static Unity.Mathematics.math;

namespace MaxMath.Tests
{
    public static class f_isnan
    {
        [Test]
        public static void _quarter()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter q = asquarter(rng.NextByte());

                Assert.AreEqual(isnan(q), isnan((float)q));
            }
        }

        [Test]
        public static void _quarter2()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter2 q = asquarter(rng.NextByte2());

                Assert.AreEqual(isnan(q), isnan((float2)q));
            }
        }

        [Test]
        public static void _quarter3()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter3 q = asquarter(rng.NextByte3());

                Assert.AreEqual(isnan(q), isnan((float3)q));
            }
        }

        [Test]
        public static void _quarter4()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter4 q = asquarter(rng.NextByte4());

                Assert.AreEqual(isnan(q), isnan((float4)q));
            }
        }

        [Test]
        public static void _quarter8()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter8 q = asquarter(rng.NextByte8());

                Assert.AreEqual(isnan(q), isnan((float8)q));
            }
        }

        [Test]
        public static void _quarter16()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter16 q = asquarter(rng.NextByte16());

                Assert.AreEqual(isnan(q).v8_0, isnan((float8)q.v8_0));
                Assert.AreEqual(isnan(q).v8_8, isnan((float8)q.v8_8));
            }
        }

        [Test]
        public static void _quarter32()
        {
            Random8 rng = Random8.New;

            for (int i = 0; i < 64; i++)
            {
                quarter32 q = asquarter(rng.NextByte32());

                Assert.AreEqual(isnan(q).v8_0, isnan((float8)q.v8_0));
                Assert.AreEqual(isnan(q).v8_8, isnan((float8)q.v8_8));
                Assert.AreEqual(isnan(q).v8_16, isnan((float8)q.v8_16));
                Assert.AreEqual(isnan(q).v8_24, isnan((float8)q.v8_24));
            }
        }


        [Test]
        public static void _half()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half q = ashalf(rng.NextUShort());

                Assert.AreEqual(isnan(q), isnan((float)q));
            }
        }

        [Test]
        public static void _half2()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half2 q = ashalf(rng.NextUShort2());

                Assert.AreEqual(isnan(q), isnan((float2)q));
            }
        }

        [Test]
        public static void _half3()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half3 q = ashalf(rng.NextUShort3());

                Assert.AreEqual(isnan(q), isnan((float3)q));
            }
        }

        [Test]
        public static void _half4()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half4 q = ashalf(rng.NextUShort4());

                Assert.AreEqual(isnan(q), isnan((float4)q));
            }
        }

        [Test]
        public static void _half8()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half8 q = ashalf(rng.NextUShort8());

                Assert.AreEqual(isnan(q), isnan((float8)q));
            }
        }

        [Test]
        public static void _half16()
        {
            Random16 rng = Random16.New;

            for (int i = 0; i < 64; i++)
            {
                half16 q = ashalf(rng.NextUShort16());

                Assert.AreEqual(isnan(q).v8_0, isnan((float8)q.v8_0));
                Assert.AreEqual(isnan(q).v8_8, isnan((float8)q.v8_8));
            }
        }
    }
}
