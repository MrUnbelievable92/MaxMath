    ...
    public static class f_PROMISE_perm
    {
        ...
        public static void _byte2()
        {
            ...
            for (int i = 0; i < 24; i++)
            {
                ...
                Assert.AreEqual(math.perm(n, k), math.perm(n, k, Promise.Unsafe0));

                n = rng.NextByte2(0, MAX_INVERSE_FACTORIAL_U32 + 1);
                k = rng.NextByte2(0, n + 1);
                Assert.AreEqual(math.perm(n, k), math.perm(n, k, Promise.Unsafe1));

                n = rng.NextByte2(0, MAX_INVERSE_FACTORIAL_U16 + 1);
                k = rng.NextByte2(0, n + 1);
                Assert.AreEqual(math.perm(n, k), math.perm(n, k, Promise.Unsafe2));

                n = rng.NextByte2(0, MAX_INVERSE_FACTORIAL_U8 + 1);
                k = rng.NextByte2(0, n + 1);
                Assert.AreEqual(math.perm(n, k), math.perm(n, k, Promise.Unsafe3));
            }
        }
