    ...
    unsafe public static class f_min
    {
        ...
        public static void _quarter2()
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                quarter2 test = math.min(a, b);

                for (int j = 0; j < 2; j++)
                {
                    Assert.AreEqual(test[j], math.min(a[j], b[j]));
                }
            }
        }
