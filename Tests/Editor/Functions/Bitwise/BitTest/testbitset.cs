...
{
    unsafe public static class f_testbitset
    {
        private static bool bts(ref UInt128 x, int i)
        {
        ...
        private static bool bts(ref ulong x, int i)
        {
        ...
        private static bool bts(ref uint x, int i)
        {
            uint mask = (uint)1 << i;
            bool ret = (x & mask) != 0;
            x |= mask;
        ...
        private static bool bts(ref ushort x, int i)
        {
            uint mask = (uint)1 << i;
            bool ret = (x & mask) != 0;
            x |= (ushort)mask;
        ...
        private static bool bts(ref byte x, int i)
        {
            uint mask = (uint)1 << i;
            bool ret = (x & mask) != 0;
            x |= (byte)mask;
        ...
        public static void _UInt128()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                for (int j = 0; j < 128; j++)
                {
                    ...
                    Assert.AreEqual(bts(ref val, j), test);
                    Assert.IsTrue(maxmath.testbit(val, (uint)j));
                }
            }
        }
        ...
        public static void _byte2()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                {
                    byte b = val[k];
                    Assert.AreEqual(bts(ref b, idx[k]), res[k]);
                    val[k] = b;
                    Assert.IsTrue(maxmath.testbit(val[k], idx[k]));
                }
            }
        }
        ...
        public static void _uint2()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                {
                    uint b = val[k];
                    Assert.AreEqual(bts(ref b, (int)idx[k]), res[k]);
                    val[k] = b;
                    Assert.IsTrue(maxmath.testbit(val[k], idx[k]));
                }
            }
        ...
        public static void _ulong()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                for (int j = 0; j < 64; j++)
                {
                    ...
                    Assert.AreEqual(bts(ref val, j), test);
                    Assert.IsTrue(maxmath.testbit(val, (ulong)j));
                }
            }
        }
