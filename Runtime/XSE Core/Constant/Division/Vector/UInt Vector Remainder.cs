...
namespace MaxMath.Intrinsics
{
    unsafe public static partial class Xse
	{
		...
		{
			internal static v128 rem_epu32(v128 vector, v128 divisor, byte elements = 4)
			{
                if (Sse2.IsSse2Supported)
                {
					if (constexpr.ALL_SAME_EPU32(divisor, elements))
					{
						return rem_epu32(vector, divisor.UInt0);
					}
			...
			internal static v256 mm256_rem_epu32(v256 vector, v256 divisor)
			{
				if (constexpr.ALL_SAME_EPU32(divisor))
				{
					return mm256_rem_epu32(vector, divisor.UInt0);
				}
