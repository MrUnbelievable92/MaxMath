...
namespace MaxMath.Intrinsics
{
	unsafe public static partial class Xse
	{
		...
		{
			internal static v128 div_epu16(v128 vector, v128 divisor, byte elements = 8)
			{
                if (Sse2.IsSse2Supported)
                {
					if (constexpr.ALL_SAME_EPU16(divisor, elements))
					{
						return div_epu16(vector, divisor.UShort0);
					}
			...
			internal static v256 mm256_div_epu16(v256 vector, v256 divisor)
			{
				if (constexpr.ALL_SAME_EPU16(divisor))
				{
					return mm256_div_epu16(vector, divisor.UShort0);
				}
