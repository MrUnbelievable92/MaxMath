...
namespace MaxMath.Intrinsics
{
    unsafe public static partial class Xse
	{
		...
		{
			internal static v128 div_epu64(v128 vector, v128 divisor)
			{
				if (constexpr.ALL_SAME_EPU64(divisor))
				{
					return div_epu64(vector, divisor.ULong0);
				}

				return new v128((ulong)(vector.ULong0 / divisor.ULong0), (ulong)(vector.ULong1 / divisor.ULong1));
			}

			internal static v256 mm256_div_epu64(v256 vector, v256 divisor, byte elements = 4)
			{
                if (Avx2.IsAvx2Supported)
                {
					if (constexpr.ALL_SAME_EPU64(divisor, elements))
					{
						return mm256_div_epu64(vector, divisor.ULong0);
					}
