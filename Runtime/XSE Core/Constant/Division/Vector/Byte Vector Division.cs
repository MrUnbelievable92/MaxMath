...
namespace MaxMath.Intrinsics
{
	unsafe public static partial class Xse
	{
		...
		{
			internal static v128 div_epu8(v128 vector, v128 divisor, byte elements = 16)
			{
                if (Sse2.IsSse2Supported)
                {
					if (constexpr.ALL_SAME_EPU8(divisor, elements))
					{
						return div_epu8(vector, divisor.Byte0);
					}
			...
			internal static v256 mm256_div_epu8(v256 vector, v256 divisor)
			{
				if (constexpr.ALL_SAME_EPU8(divisor))
				{
					return mm256_div_epu8(vector, divisor.Byte0);
				}
