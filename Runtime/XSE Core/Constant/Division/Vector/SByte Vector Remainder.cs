...
namespace MaxMath.Intrinsics
{
    unsafe public static partial class Xse
	{
		...
		{
			internal static v128 rem_epi8(v128 vector, v128 divisor, byte elements = 16)
			{
                if (Sse2.IsSse2Supported)
                {
					if (constexpr.ALL_SAME_EPI8(divisor, elements))
					{
						return rem_epi8(vector, divisor.SByte0);
					}
			...
			internal static v256 mm256_rem_epi8(v256 vector, v256 divisor)
			{
				if (constexpr.ALL_SAME_EPI8(divisor))
				{
					return mm256_rem_epi8(vector, divisor.SByte0);
				}
