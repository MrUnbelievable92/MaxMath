...
namespace MaxMath.Intrinsics
{
    unsafe public static partial class Xse
	{
		...
		public static v128 constrem_epi64(v128 vector, v128 divisor)
		{
            if (BurstArchitecture.IsSIMDSupported)
			{
				if (constexpr.ALL_SAME_EPI64(divisor))
				{
					return constrem_epi64(vector, divisor.SLong0);
				}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static v256 mm256_constrem_epi64(v256 vector, v256 divisor, byte elements = 4)
		{
		    if (Avx2.IsAvx2Supported)
		    {
				if (constexpr.ALL_SAME_EPI64(divisor, elements))
				{
					return mm256_constrem_epi64(vector, divisor.SLong0);
				}
