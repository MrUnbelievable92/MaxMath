...
namespace MaxMath
{
    namespace Intrinsics
    {
        unsafe public static partial class Xse
        {
            ...
            public static v128 subadd_epi8(v128 a, v128 b, byte elements = 16)
            {
                ...
                {
                    return add_epi8(a, sign_epi8(b, new v128(1, 255, 1, 255, 1, 255, 1, 255, 1, 255, 1, 255, 1, 255, 1, 255)));
                }
                else if (BurstArchitecture.IsSIMDSupported)
                {
                    ...
                    {
                        sub = bsrli_si128(sub, sizeof(byte));
                        return unpacklo_epi8(add, sub);
                    }
                    else
                    {
                        return blendv_si128(add, sub, new v128(0, 255, 0, 255, 0, 255, 0, 255, 0, 255, 0, 255, 0, 255, 0, 255));
            ...
            public static v128 subadd_epi16(v128 a, v128 b, byte elements = 8)
            {
                if (Avx.IsAvxSupported) // This is only always as fast as the SSSE3 version with the VEX prefix
                {
                    ...
                    {
                        return add_epi16(a, sign_epi16(b, new ushort8(1, ushort.MaxValue, 1, ushort.MaxValue, 1, ushort.MaxValue, 1, ushort.MaxValue)));
                    }
                ...
                else if (BurstArchitecture.IsSIMDSupported)
                {
                    ...
                    {
                        sub = bsrli_si128(sub, sizeof(ushort));
                        return unpacklo_epi16(add, sub);
                    }
                    else
                    {
                        return blend_epi16(add, sub, 0b1010_1010);
            ...
            public static v128 subadd_epi32(v128 a, v128 b, byte elements = 4)
            {
                if (Avx.IsAvxSupported) // This is only always as fast as the SSSE3 version with the VEX prefix
                {
                    if (constexpr.IS_CONST(b))
                    {
                        return add_epi32(a, sign_epi32(b, new v128(1, uint.MaxValue, 1, uint.MaxValue)));
                    }
                    ...
                    return blend_epi16(add, sub, 0b1100_1100);
                }
                else if (Ssse3.IsSsse3Supported)
                {
                    return add_epi32(a, sign_epi32(b, new v128(1, uint.MaxValue, 1, uint.MaxValue)));
                ...
                else if (BurstArchitecture.IsSIMDSupported)
                {
                    ...
                    {
                        sub = bsrli_si128(sub, sizeof(uint));
                        return unpacklo_epi32(add, sub);
                    }
                    else
                    {
                        return blendv_si128(add, sub, new v128(0, uint.MaxValue, 0, uint.MaxValue));
            ...
            public static v128 subadd_epi64(v128 a, v128 b)
            {
                if (BurstArchitecture.IsSIMDSupported)
                {
                    ...
                    if (BurstArchitecture.IsBlendSupported)
                    {
                        return blend_epi16(add, sub, 0b1111_0000);
                    }
                    else
                    {
                        sub = bsrli_si128(sub, sizeof(ulong));
                        return unpacklo_epi64(add, sub);
                    }
                }
                else throw new IllegalInstructionException();
            }
    ...
    unsafe public static partial class math
    {
        ...
        public static double3 addsub(double3 a, double3 b)
        {
            if (Avx.IsAvxSupported)
            {
                return Xse.mm256_subadd_pd(a, b);
            }
            else
            {
