...
namespace MaxMath
{
    namespace Intrinsics
    {
        unsafe public static partial class Xse
        {
            ...
            public static v128 avg_epi8(v128 a, v128 b, byte elements = 16)
            {
                if (BurstArchitecture.IsSIMDSupported)
                {
                    if (elements <= 8)
                    {
                        ...
                        avg = add_epi16(avg, and_si128(neg_epi16(addOneMask), add_epi16(a, b)));
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static v128 avg_epi16(v128 a, v128 b, bool promise = false)
            {
                if (BurstArchitecture.IsSIMDSupported)
                {
                    if (promise)
            ...
            public static v256 mm256_avg_epi32(v256 a, v256 b, bool promise = false)
            {
                if (Avx2.IsAvx2Supported)
                {
                    ...
                    else
                    {
                        ...
                        sumLo = Avx2.mm256_sub_epi64(sumLo, Avx2.mm256_cmpgt_epi64(sumLo, mm256_setall_si256()));
                        sumHi = Avx2.mm256_sub_epi64(sumHi, Avx2.mm256_cmpgt_epi64(sumHi, mm256_setall_si256()));
            ...
            public static v128 avg_epi64(v128 a, v128 b, bool promise = false)
            {
                if (BurstArchitecture.IsSIMDSupported)
                {
                    ...
                    else
                    {
                        ...
                        v128 addOneMask = cmpgt_epi64(sum, setall_si128());
                        avg = add_epi64(avg, and_si128(neg_epi64(addOneMask), add_epi64(a, b)));
            ...
            public static v256 mm256_avg_epi64(v256 a, v256 b, byte elements = 4, bool promise = false)
            {
                if (Avx2.IsAvx2Supported)
                {
                    if (promise)
                    {
                        ...
                        v256 addOneMask = Avx2.mm256_cmpgt_epi64(sum, mm256_setall_si256());
                    ...
                    else
                    {
                        ...
                        v256 addOneMask = Avx2.mm256_cmpgt_epi64(sum, mm256_setall_si256());
                        avg = Avx2.mm256_add_epi64(avg, Avx2.mm256_and_si256(mm256_neg_epi64(addOneMask), Avx2.mm256_add_epi64(a, b)));
    ...
    unsafe public static partial class math
    {
        ...
        public static Int128 avg(Int128 x, Int128 y, Promise noOverflow = Promise.Nothing)
        {
            ...
            else
            {
                ...
                {
                    x += andnot(tobyte(x != MaxMath.Int128.MaxValue), sumS.hi64 >> 63);
                }
                else
                {
                    y += andnot(tobyte(y != MaxMath.Int128.MaxValue), sumS.hi64 >> 63);
                }
        ...
        public static short2 avg(short2 x, short2 y, Promise noOverflow = Promise.Nothing)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                return Xse.avg_epi16(x, y, noOverflow.Promises(Promise.NoOverflow));
            }
            else
            {
        ...
        public static uint2 avg(uint2 x, uint2 y, Promise noOverflow = Promise.Nothing)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                return Xse.avg_epu32(x, y, noOverflow.Promises(Promise.NoOverflow));
            }
            else
            {
                if (noOverflow.Promises(Promise.NoOverflow))
        ...
        public static int2 avg(int2 x, int2 y, Promise noOverflow = Promise.Nothing)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                return Xse.avg_epi32(x, y, noOverflow.Promises(Promise.NoOverflow), 2);
            }
            else
            {
        ...
        public static int3 avg(int3 x, int3 y, Promise noOverflow = Promise.Nothing)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                return Xse.avg_epi32(x, y, noOverflow.Promises(Promise.NoOverflow), 3);
            }
            else
            {
        ...
        public static int4 avg(int4 x, int4 y, Promise noOverflow = Promise.Nothing)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                return Xse.avg_epi32(x, y, noOverflow.Promises(Promise.NoOverflow), 4);
            }
            else
            {
        ...
        public static double avg(double x, double y)
        {
            if (constexpr.IS_CONST(x))
            {
                return mad(0.5d, y, 0.5d * x);
            }
            if (constexpr.IS_CONST(y))
            {
                return mad(0.5d, x, 0.5d * y);
            }
