...
namespace MaxMath
{
    unsafe public static partial class math
    {
        ...
        public static float exp2(int x, Promise noOverflow = Promise.Nothing)
        {
            if (!noOverflow.Promises(Promise.NoOverflow))
            {
                if (BurstArchitecture.IsMinMaxSupported)
                {
                    ...
                    v128 MAX_EXPONENT = Xse.cvtsi32_si128((abs(F32_EXPONENT_BIAS) + 1) << F32_MANTISSA_BITS);
                    v128 BIAS         = Xse.cvtsi32_si128(abs(F32_EXPONENT_BIAS)       << F32_MANTISSA_BITS);
                ...
                {
                    x = clamp(x, nabs(F32_EXPONENT_BIAS), abs(F32_EXPONENT_BIAS) + 1);
                }
            }

            return asfloat((abs(F32_EXPONENT_BIAS) << F32_MANTISSA_BITS) + (x << F32_MANTISSA_BITS));
        }
        ...
        public static float exp2(uint x, Promise noOverflow = Promise.Nothing)
        {
            if (!noOverflow.Promises(Promise.NoOverflow))
            {
                x = min(x, (uint)abs(F32_EXPONENT_BIAS) + 1);
            }
        ...
        public static double exp2(long x, Promise noOverflow = Promise.Nothing)
        {
            if (!noOverflow.Promises(Promise.NoOverflow))
            {
                x = clamp(x, nabs(F64_EXPONENT_BIAS), abs(F64_EXPONENT_BIAS) + 1);
            }

            return asdouble(((long)abs(F64_EXPONENT_BIAS) << F64_MANTISSA_BITS) + (x << F64_MANTISSA_BITS));
        }
        ...
        public static double exp2(ulong x, Promise noOverflow = Promise.Nothing)
        {
            if (!noOverflow.Promises(Promise.NoOverflow))
            {
                x = min(x, (uint)abs(F64_EXPONENT_BIAS) + 1);
            }
