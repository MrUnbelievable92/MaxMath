...
namespace MaxMath
{
    namespace Intrinsics
    {
        unsafe public static partial class Xse
        {
            ...
            public static v128 adds_epu32(v128 a, v128 b, byte elements = 4)
            {
                ...
                {
                    return Arm.Neon.vqaddq_u32(a, b);
                }
                else if (Sse2.IsSse2Supported)
                {
                    ...
                    return or_si128(sum, overflowMask);
                }
                else throw new IllegalInstructionException();
            }
            ...
            public static v128 adds_epu64(v128 a, v128 b)
            {
                if (Arm.Neon.IsNeonSupported)
                {
                    return Arm.Neon.vqaddq_u64(a, b);
                }
                else if (Sse2.IsSse2Supported)
                {
            ...
            public static v128 adds_epi32(v128 a, v128 b, byte elements = 4)
            {
                if (Arm.Neon.IsNeonSupported)
                {
                    return Arm.Neon.vqaddq_s32(a, b);
                }
                else if (Sse2.IsSse2Supported)
                {
                    ...
                    if (constexpr.ALL_EQ_EPI32(b, 1, elements))
                    {
                        return sub_epi32(a, cmpgt_epi32(MAX_VALUE, a));
                    }


                    v128 result = add_epi32(a, b);
                    ...
                    return blendv_ps(overflow, result, selectResult);
                }
                else throw new IllegalInstructionException();
            }
            ...
            public static v128 adds_epi64(v128 a, v128 b)
            {
                if (Arm.Neon.IsNeonSupported)
                {
                    return Arm.Neon.vqaddq_s64(a, b);
                }
                else if (Sse2.IsSse2Supported)
                {
                    ...
                    if (constexpr.ALL_EQ_EPI64(b, 1))
                    {
                        return sub_epi64(a, cmpgt_epi64(MAX_VALUE, a));
                    }


                    v128 result = add_epi64(a, b);
                    v128 overflow = add_epi64(MAX_VALUE, srli_epi64(a, 63));
                    v128 selectResult = ternarylogic_si128(overflow, b, result, TernaryOperation.OxBD);

                    return blendv_pd(overflow, result, selectResult);
                }
                else throw new IllegalInstructionException();
            }
    ...
    unsafe public static partial class math
    {
        ...
        {
            if (constexpr.IS_TRUE(x <= byte.MaxValue / 2 && y <= byte.MaxValue / 2))
            {
                return (byte)(x + y);
            }
            if (constexpr.IS_TRUE(x == 1))
        ...
        {
            if (constexpr.IS_TRUE(x <= ushort.MaxValue / 2 && y <= ushort.MaxValue / 2))
            {
                return (ushort)(x + y);
            }
            if (constexpr.IS_TRUE(x == 1))
        ...
        {
            if (constexpr.IS_TRUE(x <= uint.MaxValue / 2 && y <= uint.MaxValue / 2))
            {
                return x + y;
            }
            if (constexpr.IS_TRUE(x == 1))
        ...
        {
            if (constexpr.IS_TRUE(x <= ulong.MaxValue / 2 && y <= ulong.MaxValue / 2))
            {
                return x + y;
            }
            if (constexpr.IS_TRUE(x == 1))
        ...
        public static sbyte addsaturated(sbyte x, sbyte y)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                ...
                v128 sum = Xse.adds_epi8(_x, _y);

                return (sbyte)Xse.cvtsi128_si32(sum);
            }
            else
            {
        ...
        public static short addsaturated(short x, short y)
        {
            if (BurstArchitecture.IsSIMDSupported)
            {
                ...
                v128 sum = Xse.adds_epi16(_x, _y);

                return (short)Xse.cvtsi128_si32(sum);
            }
            else
            {
