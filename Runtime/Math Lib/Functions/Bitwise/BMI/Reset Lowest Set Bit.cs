...
namespace MaxMath
{
    ...
    unsafe public static partial class maxmath
    {
        ...
        public static uint2 bits_resetlowest(uint2 x)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint2>(Xse.blsr_epi32(RegisterConversion.ToV128(x)));
            }
            else
            {
        ...
        public static uint3 bits_resetlowest(uint3 x)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint3>(Xse.blsr_epi32(RegisterConversion.ToV128(x)));
            }
            else
            {
        ...
        public static uint4 bits_resetlowest(uint4 x)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint4>(Xse.blsr_epi32(RegisterConversion.ToV128(x)));
            }
            else
            {
