...
namespace MaxMath
{
    ...
    unsafe public static partial class maxmath
    {
        ...
        public static uint2 bits_depositparallel(uint2 x, uint2 mask)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint2>(Xse.pdep_epi32(RegisterConversion.ToV128(x), RegisterConversion.ToV128(mask), 2));
            }
            else
            {
        ...
        public static uint3 bits_depositparallel(uint3 x, uint3 mask)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint3>(Xse.pdep_epi32(RegisterConversion.ToV128(x), RegisterConversion.ToV128(mask), 3));
            }
            else
            {
        ...
        public static uint4 bits_depositparallel(uint4 x, uint4 mask)
        {
            if (Sse2.IsSse2Supported)
            {
                return RegisterConversion.ToType<uint4>(Xse.pdep_epi32(RegisterConversion.ToV128(x), RegisterConversion.ToV128(mask), 4));
            }
            else
            {
