...
- *Vector Width:* It is recommended, just like with Unity.Mathematics, to use vector types that use up an entire SIMD register (128 and 256 bits, respectively). LLVM has a very hard time optimizing code which does not follow this recommendation
- *Boolean Vectors:* Wherever possible, it is adviced not to declare boolean vectors as temporary variables on the stack. MaxMath uses a 1:1 representation of SIMD masks for boolean data in the form of `ref struct`s named `mask[BITS]x[COLUMNS]x[ROWS]`. They are implicitly convertible to and from boolean vectors and matrices and should therefore be invisible to the user and cannot leave the stack, so the expected behaviour from using the Unity.Mathematics and earlier MaxMath API is still guaranteed, yet converting to and from boolean types comes at a performance cost already present in both Unity.Mathematics (although rarer) and MaxMath. Since the mask types are `public` and have the exact same API as corresponding boolean types, you are free to use them instead, yet their obscurity through specialization opens up a larger surface area for programmer error.
- *Math Functions as optimizing abstractions:* One of the core strengths of MaxMath is that it provides operations whose implementations are better than what a compiler can synthesize from primitive operators; prefer using dedicated MaxMath operations over composing equivalent expressions manually. Many functions map to specialized SIMD instructions or architecture-specific implementations that cannot be recognized reliably by Burst or LLVM, and communicate mathematically proven metainformation back to the compiler, like value ranges, which feed back into ubiquitous compile-time checks throughout MaxMath that select optimized code paths at compile time. If you are able to exploit specialized SIMD instructions for mathematical algorithms, consider submitting the resulting code to MaxMath as a pull-request. Otherwise you are very much welcome to open a feature request via the issue tracker.
- *Auto-Vectorization:* Unlike `Unity.Mathematics.math`, scalar `MaxMath.math` functions sometimes do not get optimized by LLVM's vectorizer. Burst provides LLVM with vectorization information for scalar `Unity.Mathematics.math` functions via vector function mappings ([llvm::VFABI](https://llvm.org/doxygen/namespacellvm_1_1VFABI.html)) that the loop vectorizer can use to replace scalar function calls with their vector equivalents. Since MaxMath functions are not part of this mapping, they typically will not auto-vectorize. For maximum performance, prefer explicit SIMD code using MaxMath vector types. In performance-critical code, vectors should be loaded and stored directly with `unsafe` pointer operations to minimize overhead.
